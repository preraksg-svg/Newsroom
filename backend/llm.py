...
def clean_headline_garbage(title):
    ...
    # are never touched.
    # NOTE: the source name may be multi-word (e.g. "The Economic Times.com"), but
    # it must END in a real domain TLD anchored at "$", so a normal headline that
    # merely contains a hyphen cannot be truncated — only a real trailing
    # "Source.com"-style attribution is removed. The source name is words joined
    # by single spaces or hyphens (never a " - " separator), so the match cannot
    # bridge an earlier spaced separator and swallow real headline words.
    cleaned = re.sub(
        r'\s*[\-\|\—\–\/]\s*[A-Za-z0-9]+(?:[.&-][A-Za-z0-9]+|\s+[A-Za-z0-9]+)*\.(?:com|in|net|org|co|news|io)\s*$',
